...
Gui::Gui(int width, int height) : impl(new Impl) {
    ...
    TTF_Init();
    impl->window = SDL_CreateWindow("五子棋 Gomoku", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, 0);
...
void Gui::run(Game& game, bool vsAI) {
    ...
    while (impl->running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && game.status() == GameStatus::Playing) {
